<?php
    ...
    if(!isset($_SESSION['USU_RUT'])){
        ...
        //selecciono zona horaria y almaceno hora y fecha por separado
        date_default_timezone_set("America/Santiago");
        $fecha = date("Y-m-d");
        $hora = date("H:i:s");
        $id_formulario = 12; //id asigana al formulario
        $ipcliente = getRealIP(); //guardo ip del cliente
        //busco la firma del usuario
        $queryFirma = "SELECT USU_RUT, USU_FIR FROM USUARIO WHERE (USU_RUT = '".$Srut."'')";
        $rsqFirma = mysqli_query($cnn, $queryFirma);
        if (mysqli_num_rows($rsqFirma) != 0){
            $rowqFirma = mysqli_fetch_row($rsqFirma);
            if ($rowqFirma[0] == $Srut){
                $Mfirma = $rowqFirma[14];
            }
        }
        //consulto el estado del formulario
            ...
            <div class="section">
                <div class="row">
                    <div class="col s12 center block" style="background-color: #ffffff">
                        <h4 class="light">Firma Digital</h4>
                        </br></br></br></br>
                        <div class="col s12">
                        <?php 
                            if ($Mfirma == ""){
                                echo "<h6>USUARIO NO TIENE FIRMA</h6>";
                            }else{
                                echo "<h6>SU FIRMA ES : </h6>";
                                echo "<h6>".$Mfirma."</h6>";
                            }
                        ?>
                        </div>
                        </br></br></br></br>
                        <form name="f_firma" id="firma" enctype="multipart/form-data" method="post">
