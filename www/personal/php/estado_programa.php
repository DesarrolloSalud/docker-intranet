<?php
    ...
    if(!isset($_SESSION['USU_RUT'])){
	    ...
	    $programa = "SELECT OP_ID,OP_NOM FROM OTE_PROGRAMA WHERE (OP_ID = $id)";
	    $resPro = mysqli_query($cnn, $programa);
	    if (mysqli_num_rows($resPro) != 0){
	    	$rowP = mysqli_fetch_row($resPro);
	        if ($rowP[0] == $id){
	        	$nombrePrograma = utf8_encode($rowP[1]);
	        }
	    }
	    ...
	    $id_formulario = 31;
	    $ipcliente = getRealIP();
	    $accionRealizada = utf8_decode($estado." Programa : ".$nombrePrograma);
		...
		$actualizar = "UPDATE OTE_PROGRAMA SET OP_ESTA = '$estado' WHERE (OP_ID = $id)";
