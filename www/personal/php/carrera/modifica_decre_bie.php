<?php
    ...
    if(!isset($_SESSION['USU_RUT'])){
        ...
        $busca_deta = "SELECT BA_ID,BA_DC_NUM FROM DECRE_BIE WHERE (DB_ID = '$idmod')";    
         $respuesta = mysqli_query($cnn, $busca_deta);
            if($row1 = mysqli_fetch_array($respuesta)){
                $MuestroNum   = $row1[1];            
            }
          
          $modifica_enc = "UPDATE DECRE_BIE SET DB_DC_NUM ='$dcnum', DB_FEC='$dcfec', DB_VISTO = '$dcvisto', DB_CONSI='$dcconsi', DB_DEC='$dcdec', DB_ALCALDE='$alcalde', DB_SECRE='$secretaria', 
          DB_DERIVA='$distribucion',DB_FEC_CONT = '$fechasub', DB_ACT_TOTAL ='$puncap', DB_BIE_PUN ='$biepun', DB_ALCSUB='$alcaldesub', DB_SECSUB='$secretariasub', DB_GENALC='$genalcalde', DB_GENSEC='$gensecre' WHERE (DB_ID = '$idmod') AND (USU_RUT = '$usurut')";    
          mysqli_query($cnn,$modifica_enc);
      
          $modifica_deta ="UPDATE DECRE_BIE_DETA SET DB_DC_NUM = '$dcnum' WHERE (DB_DC_NUM='$MuestroNum') AND (USU_RUT = '$usurut')";
         ...
         mysqli_query($cnn, $actualizanivel);
            ...
            //$id_formulario = 25;
            $ipcliente = getRealIP();
            $accionRealizada = utf8_decode("MODIFICA DECRETO NIVEL :  ID :".$idmod." ".$nomusu." ".$appusu." ".$apmusu);
