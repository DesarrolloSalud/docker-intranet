        ...
        if (mysqli_num_rows($respuesta) == 1){
            ...
            if ($rowSP[13] == "AUTORIZADO DIR"){
                ...
                if ($rowUSUR2[0] == "SI"){
                    ...
                    $año_actual = date("Y");
                    $query_banco_adm = "SELECT BD_ID, BD_ADM,BD_ADM_USADO FROM BANCO_DIAS WHERE (USU_RUT = '$Srut') AND (BD_ANO = '$año_actual')";
                    ...
                    if($sp_año == $año_actual){
                        $query_banco_fl = "SELECT BD_ID, BD_FL, BD_FLA, BD_FL_USADO FROM BANCO_DIAS WHERE (USU_RUT = '$Srut') AND (BD_ANO = '$año_actual')";
                        ...
                        $query_banco_fl = "SELECT BD_ID, BD_FL, BD_FLA, BD_FL_USADO FROM BANCO_DIAS WHERE (USU_RUT = '$Srut') AND (BD_ANO = '$año_siguiente')";
                        $resultado_banco_fl = mysqli_query($cnn, $query_banco_fl);
                        if (mysqli_num_rows($resultado_banco_fl) != 0){
                            while ($row_fl = mysqli_fetch_array($resultado_banco_fl)){
                                $num_fl         = $row_fl[1];
                    ...
                    $query_banco_hora = "SELECT BH_SALDO,BH_CANT FROM BANCO_HORAS WHERE (USU_RUT = '$Srut') AND (BH_SALDO > 0) AND (BH_FEC BETWEEN '$FecIni' AND '$FecActual') AND ((BH_TIPO = 'INICIAL') OR (BH_TIPO = 'INGRESO')) ORDER BY BH_FEC ASC";
