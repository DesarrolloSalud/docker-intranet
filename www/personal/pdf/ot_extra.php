    ...
    if(!isset($_SESSION['USU_RUT'])){
        ...
        if (mysqli_num_rows($respuesta) == 1){
            ...
            if ($rowOE[10] == "V.B. DIR SALUD"){
                ...
                $pdf->Ln(10);
                $pdf->SetFontSize(10);
                $pdf->Write(5,utf8_decode('Se ordena los siguientes trabajos extraordinarios :'));
                $pdf->Ln(10);
                $pdf->Write(5,'Para el Funcionario : '.$usu_nom." ".$usu_app." ".$usu_apm);
                $pdf->Ln(5);
                $pdf->Write(5,utf8_decode('Categoría : ').$usu_cat);
                $pdf->Ln(10);
                $pdf->Write(5,'Para cumplir el trabajo de : '.$oe_trab);
                $pdf->Ln(15);
                $pdf->Write(5,utf8_decode('Según lo siguiente : '));
                $pdf->Ln(10);
                $DetalleMiOrden = "SELECT DATE_FORMAT(OTE_DIA,'%d-%m-%Y'),OTE_HORA_INI,OTE_HORA_FIN,OTE_DIA,OTE_TIPO,OTE_ESTA FROM OTE_DETALLE WHERE (OE_ID = $oe_id) ORDER BY OTE_DIA ASC, OTE_TIPO";
