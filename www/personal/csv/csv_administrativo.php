...
if(!isset($_SESSION['USU_RUT'])){
  ...
  while ($rowDEC = mysqli_fetch_array($respuestaQuery, MYSQLI_NUM)){
    $folio_doc = $rowDEC[0];
    $feriado = "SELECT S.USU_RUT,U.USU_NOM,U.USU_APP,U.USU_APM,DATE_FORMAT(S.SP_FEC_INI,'%d/%m/%Y'),DATE_FORMAT(S.SP_FEC_FIN,'%d/%m/%Y'),S.SP_CANT_DIA FROM SOL_PERMI S INNER JOIN USUARIO U ON S.USU_RUT = U.USU_RUT WHERE S.SP_ID = $folio_doc";
    ...
    $csv.=$A.$csv_sep.$B.$csv_sep.$rut.$csv_sep.$dv.$csv_sep.$funcionario.$csv_sep.$F.$csv_sep.$G.$csv_sep.$H.$csv_sep.$I.$csv_sep.$J.$csv_sep.$df_num.$csv_sep.$df_fec.$csv_sep.$M.$csv_sep.$sp_fec_ini.$csv_sep.$sp_fec_fin.$csv_sep.$P.$csv_sep.$Q.$csv_sep.$R.$csv_sep.$csv_sep.$csv_sep.$csv_sep.$csv_sep.$csv_sep.$csv_sep.$csv_sep.$csv_sep.$csv_sep.$csv_sep.$csv_end;
  }
header("Content-Description: File Transfer");
header("Content-Type: application/force-download");
header("Content-Disposition: attachment; filename=administrativo.csv");
echo $csv;
}
